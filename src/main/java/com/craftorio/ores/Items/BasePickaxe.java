...
public class BasePickaxe extends ItemPickaxe implements IHasCustomModel, IColoredItem {
    ...
    public BasePickaxe(ToolMaterial t, int color) {
        super(t);
        this.color = color;
        this.setTranslationKey(Ores.MODID + ":pickaxe" + t.name());
        OreLogger.localize(this.getTranslationKey() + ".name=" + t.name().substring(0, 1).toUpperCase() + t.name().substring(1) + " Pickaxe");
