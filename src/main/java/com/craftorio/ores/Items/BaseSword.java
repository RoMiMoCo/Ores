...
public class BaseSword extends ItemSword implements IHasCustomModel, IColoredItem {
    ...
    public BaseSword(ToolMaterial t, int color) {
        super(t);
        this.color = color;
        this.setTranslationKey(Ores.MODID + ":sword" + t.name());

        OreLogger.localize(this.getTranslationKey() + ".name=" + t.name().substring(0, 1).toUpperCase() + t.name().substring(1) + " Sword");
    }
